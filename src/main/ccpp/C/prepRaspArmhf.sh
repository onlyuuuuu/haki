...
echo 0 | sudo -S cp -rf $BASE_DIR/cmdline.ubuntu.updated.txt $BASE_DIR/mnt/boot/cmdline.txt
echo 0 | sudo -S cp -rf $BASE_DIR/mnt/boot/bcm2709-rpi-2-b.dtb $BASE_DIR/bcm2709-rpi-2-b.dtb
echo 0 | sudo -S cp -rf $BASE_DIR/mnt/boot/vmlinuz $BASE_DIR/vmlinuz
echo 0 | sudo -S umount mnt/boot
echo 0 | sudo -S umount mnt/root 2> /dev/null
echo 0 | sudo -S mount -o loop,offset=537919488 ubuntu-22.04.5-preinstalled-server-armhf+raspi.img mnt/root
# TODO: Actually do something meaningful
echo "Root mounted, but not doing anything yet"
echo 0 | sudo -S umount mnt/root
