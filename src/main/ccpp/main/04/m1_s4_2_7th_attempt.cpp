...
int main()
{
    ...
    multiset<int>out;multiset<int>::const_iterator it;
    ...
    for (;ita!=A.cend();++ita)
    {
        if (M+ita->first>=*out.crbegin())break;
        for (const auto &x : m[!ita->second])
        {
            i=x+ita->first;
            if (i>=*out.crbegin())break;
            out.erase(--out.end());
        ...
        m[ita->second].push_back(ita->first);
    }
    for (;itb!=B.cend();++itb)
    {
        if (M+itb->first>=*out.crbegin())break;
        for (const auto &x : m[!itb->second])
        {
            i=x+itb->first;
            if (i>=*out.crbegin())break;
            out.erase(--out.end());
        ...
        m[itb->second].push_back(itb->first);
    }
    // ----------------------------------------
